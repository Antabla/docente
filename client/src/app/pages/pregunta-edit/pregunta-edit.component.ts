/* 
* Generated by
* 
*      _____ _          __  __      _     _
*     / ____| |        / _|/ _|    | |   | |
*    | (___ | | ____ _| |_| |_ ___ | | __| | ___ _ __
*     \___ \| |/ / _` |  _|  _/ _ \| |/ _` |/ _ \ '__|
*     ____) |   < (_| | | | || (_) | | (_| |  __/ |
*    |_____/|_|\_\__,_|_| |_| \___/|_|\__,_|\___|_|
*
* The code generator that works in many programming languages
*
*			https://www.skaffolder.com
*
*
* You can generate the code from the command-line
*       https://npmjs.com/package/skaffolder-cli
*
*       npm install -g skaffodler-cli
*
*   *   *   *   *   *   *   *   *   *   *   *   *   *   *   *
*
* To remove this comment please upgrade your plan here: 
*      https://app.skaffolder.com/#!/upgrade
*
* Or get up to 70% discount sharing your unique link:
*       https://beta.skaffolder.com/#!/register?friend=5d1e567d0e8b622e6162666b
*
* You will get 10% discount for each one of your friends
* 
*/
// Import Libraries
import { Component, OnInit } from '@angular/core';
import { Location } from '@angular/common';
import { ActivatedRoute } from '@angular/router';
// Import Services
import { PreguntaService } from '../../services/pregunta.service';
// Import Models
import { Pregunta } from '../../domain/docente_db/pregunta';

// START - USED SERVICES
/**
* PreguntaService.create
*	@description CRUD ACTION create
*
* PreguntaService.get
*	@description CRUD ACTION get
*	@param ObjectId id Id 
*
* PreguntaService.update
*	@description CRUD ACTION update
*	@param ObjectId id Id
*
*/
// END - USED SERVICES

/**
 * This component allows to edit a Pregunta
 */
@Component({
    selector: 'app-pregunta-edit',
    templateUrl: 'pregunta-edit.component.html',
    styleUrls: ['pregunta-edit.component.css']
})
export class PreguntaEditComponent implements OnInit {
    item: Pregunta;
    model: Pregunta;
    formValid: Boolean;

    constructor(
    private preguntaService: PreguntaService,
    private route: ActivatedRoute,
    private location: Location) {
        // Init item
        this.item = new Pregunta();
    }

    /**
     * Init
     */
    ngOnInit() {
        this.route.params.subscribe(param => {
            const id: string = param['id'];
            if (id !== 'new') {
                this.preguntaService.get(id).subscribe(item => this.item = item);
            }
            // Get relations
        });
    }


    /**
     * Save Pregunta
     *
     * @param {boolean} formValid Form validity check
     * @param Pregunta item Pregunta to save
     */
    save(formValid: boolean, item: Pregunta): void {
        this.formValid = formValid;
        if (formValid) {
            if (item._id) {
                this.preguntaService.update(item).subscribe(data => this.goBack());
            } else {
                this.preguntaService.create(item).subscribe(data => this.goBack());
            } 
        }
    }

    /**
     * Go Back
     */
    goBack(): void {
        this.location.back();
    }


}



